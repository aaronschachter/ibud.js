...
module.exports.postMessage = function (json) {
  return request({
    ...
    if (!error && response.statusCode == 200) {
      ...
      if (messageId) {
        logger.info(`Successfully sent message:${messageId} %s to recipient:${recipientId}`);
      } else {
        logger.info(`Successfully called Send API for recipient:${recipientId}`);
      }
    } else {
      logger.error(`Send API error code ${response.statusCode}:${response.statusMessage} | ${body.error}`);
    }
  });  
};
