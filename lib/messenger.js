...
module.exports.executePost = function (endpoint, data) {
  return superagent
    .post(`${uri}/${endpoint}`)
...
module.exports.postMessage = function (data) {
  logger.debug(`postMessage:${JSON.stringify(data)}`);

  return this.executePost('messages', data);
};

module.exports.postThreadSettings = function (data) {
  logger.debug(`postThreadSettings:${JSON.stringify(data)}`);

  return this.executePost('thread_settings', data);
};
